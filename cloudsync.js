...
const TCS_BUILD_VERSION = "2026-03-24.6";
  ...
  class CloudSyncApp {
    ...
    loadLocalArchiveSection(modal) {
      ...
      const renderRow = (c) => {
        ...
        return `<div class="archived-chat-row flex items-center gap-2 px-2 py-1.5 cursor-pointer hover:bg-zinc-600 text-xs text-zinc-300 border-b border-zinc-600/50" style="overflow:hidden" data-chat-id="${c.chatId}" data-search="${esc(c.title).toLowerCase()} ${c.chatId.toLowerCase()}"><span class="shrink-0 text-zinc-500" style="font-size:10px;width:62px">${datePart}</span><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1;min-width:0" title="${title}">${title}</span></div>`;
      };
